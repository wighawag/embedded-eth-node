...
> **Conductor annotation 2026-08-02 (drive-tasks Gate-3 triage), correcting decision 1 before it is ratified.** Decision 1 says "nothing in the widening restates a constant the formula contains". That is true of the TERM DEFINITIONS (`INTRINSIC_TERMS` derives every term as a delta between measured charges, including the initcode row) and NOT true of the whole widening: `test/revm-engine.spec.ts` also asserts absolute per-party anchors at `berlin` and `cancun` (21000 / 16 / 4 / 32000 / 0 and 2), and those are constants. The anchors are DEFENSIBLE and should stay — three parties agreeing on a number all three get wrong is exactly what the pre-`0.3.1` world looked like, so a relative-only check would have passed it — but the record and the code should say the same thing. Read decision 1 as "no term's DEFINITION restates a constant the formula contains; the absolute anchors deliberately do, as the cross-check against uniform agreement". Raised by Gate 2, verified at the code site.
