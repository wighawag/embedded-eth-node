...
The subclass is also cheaper than it looks: `topStorageStack()` is `protected`, so a subclass reaches the live frame through the API the base class already offers, with no cast and no reach past the type. That is strictly better than the `accountStack`/`codeStack`/`storageStack` reach-through ADR 0005 needed for the revm read path.
