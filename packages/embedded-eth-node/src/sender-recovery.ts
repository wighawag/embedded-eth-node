 ...
 * module ALREADY contains secp256k1 — `embedded-eth-node/revm`, whose `0x01`
...
export function recoverSender(
	...
	if (v === undefined || r === undefined || s === undefined) {
		throw new Error(
			'embedded-eth-node: cannot recover the sender of an UNSIGNED transaction ' +
	...
	if (common.gteHardfork('homestead') && s > SECP256K1_ORDER_DIV_2) {
		throw new Error(
			'embedded-eth-node: Invalid Signature: s-values greater than secp256k1n/2 ' +
	...
	if (recovery !== 0n && recovery !== 1n) {
		throw new Error(
			`embedded-eth-node: Invalid Signature: v ${v} is not a recovery id on chain ` +
	...
	if (address === undefined || address.length !== 20) {
		throw new Error(
			"embedded-eth-node: Invalid Signature: the engine's ecrecover recovered no " +
