...
>  (a) add a real KZG implementation (e.g. `kzg-wasm`, present in the lockfile only as a transitive dep of tevm under packages/benchmarks) as a RUNTIME dependency of `embedded-eth-node` — a trusted-setup-carrying wasm dependency in a package whose identity is "slim, in-browser", affecting every consumer's bundle whether or not they ever send a blob;
...
>  1. A DECISION item (task or ADR under docs/adr/): "does embedded-eth-node accept type-3 transactions, and in which wire form". If the answer is (c) refuse, story 15 narrows to an honest-edge refusal naming EIP-4844 and the reason, and the receipt half is dropped as unreachable rather than built.
