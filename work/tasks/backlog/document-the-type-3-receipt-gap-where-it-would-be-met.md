...
- [ ] A consumer sending a type-3 transaction on the revm path meets a stated limitation at that point: either a refusal naming the missing fields, or a receipt whose incompleteness is documented where it is produced.
...
- [ ] The default `@ethereumjs/evm` path is unaffected: whatever it does with type-3 transactions today, it still does.
