...
> **Conductor note 2026-08-02 (second pass), one more item for the citation-hygiene criterion.** ADR 0008's FIRST amendment still closes by naming `paris` as "the standing counter-example" for clause (b) — a claim the SECOND amendment made false the same day by re-admitting `paris`. It is pre-existing drift rather than anything this task caused, but this task is the one holding the pen on ADR 0008's hygiene, and the third amendment (landed by `clause-b-covers-only-eip-3860-not-the-rest-of-the-formula`) has since moved the counter-examples again: the standing ones are now the still-refused `prague`/`osaka` for clause (a), and `petersburg`/`istanbul`, measured from both sides, for the EIP-2028 boundary. Correct the stale sentence in place, in the amendment's own voice, WITHOUT rewriting the amendment's argument: an amendment records what was true when it was written, so mark or correct the one stale claim, do not retro-fit the whole section. Raised by the Gate-2 review of that task.
